...
from src.robot_control.scripts.configs.vdn_config import VDNConfig
from src.robot_control.scripts.marltoolkit.agents.vdn_agent import VDNAgent
...
from src.robot_control.scripts.marltoolkit.modules.mixers import VDNMixer
...
async def main(args):
    ...
    args.state_shape = int(get_env_info["state_shape"][0]/gz_env.n_agents) ####임의 수정. 확인 바람. 기존 코드는 get_env_info["state_shape"][1] 이지만, 현재 env는 state를 flatten해서 제공함. [1]이 존재하지 않음. 
    ...
    log_file = os.path.join(log_path, log_name + 'vdn.log')
    print(log_file)
